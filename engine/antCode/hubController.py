...
class hubController:
    ...
    def agentLeave(self,angle,id):
        self.numAgentsByDir[angle] += 1
        agent = self.agentInfoList[id]
    ...
    def beeCheckOut(self, bee):
        ...
        agent = self.agentInfoList[bee.id]
        if (self.desiredUserValsByAngle[angle] == -1):  # No user input, all is well
            self.agentLeave(angle, bee.id)
        elif self.desiredUserValsByAngle[angle] < self.numAgentsByDir[angle]:  # too many bees, stop it!
            bee.state = Waiting(bee)
            #bee.observeTransition()
        elif (self.desiredUserValsByAngle[angle] > self.numAgentsByDir[angle]):  # there needs to be more bees in that direction
            self.agentLeave(angle,bee.id)
        elif self.desiredUserValsByAngle[angle] == self.numAgentsByDir[angle]:  # perfect amount of bees, stop it
            bee.state = Waiting(bee)
            #bee.observeTransition()
    ...
    def beeCheckIn(self,bee):
        ...
        #eprint("checking in:", id, " Initial direction:", self.agentInfoList[id].direction, " from:", int(dir*(180/np.pi))+180)

        #angle calculations
        exitAngle = int(self.agentInfoList[bee.id].direction / 5)
        ...
        self.numAgentsByDir[exitAngle] = self.numAgentsByDir[exitAngle] - 1
        self.incoming[inAngle] += 1
        self.agentInfoList[bee.id].checkIn(inAngle*5, bee.state)
        self.agentsInHub[bee.id] = bee


        if self.agentInfoList[bee.id].dead:
            self.agentInfoList[bee.id].dead = False
            self.deadBees -= 1
        #if isinstance(bee.state, Piping):
        #    self.piperCount += 1

        #if (isinstance(bee.state, Exploiting) and not self.no_viewer):
    ...
    def directionInput(self, direction, newValue):  # user inhibits or excites the amount of bees in each direction
        ...
        self.desiredUserValsByAngle[angle] = int(newValue)
    ...
    def hubUpdate(self, bees):
        # sortedParams = sorted(self.desiredUserValsByAngle, operator.getitem(1), Reverse=True)

        #This code is checking for the dead bees
        if self.exploreCounter < 1 and not self.piperCheck():
            for id, beeInf in self.agentInfoList.items():
        ...
        for counter in range(0, 72):
            ...
            if self.desiredUserValsByAngle[angle] == -1:  # No user input, all is well
                pass
            elif self.desiredUserValsByAngle[angle] < self.numAgentsByDir[angle]:
                pass # too many bees, just keep stopping them from leaving
            elif self.desiredUserValsByAngle[angle] > self.numAgentsByDir[angle]:  # not enough bees send out more! from observers

                #for id, bee in bees.items():  # TODO to speed up use the in hub agents
                for id, bee in self.agentsInHub.items():
                    if bee.state.__class__ == Waiting().__class__ and bee.inHub is True:
                ...
                self.desiredUserValsByAngle[angle] = -1
            ...
            "data":
                {
                    "controller":
                        {
                            "agentDirections": self.numAgentsByDir,
    ...
    def reset(self, radius, agents, environment, exploreTime):
        ...
        self.numAgentsByDir = [None] * 72  # #bees that have left the hub in each direction
        self.desiredUserValsByAngle = [None] * 72  # desired user values

        self.agentsInHub = RandomDict()
        self.agentInfoList = {}
        for counter in range(0, 72):
            self.numAgentsByDir[counter] = 0
            self.desiredUserValsByAngle[counter] = -1
            self.incoming[counter] = 0
        # self.desiredUserValsByAngle[10] = 10
        #print (type(agents))
        for id, bee in agents.items():
            direction = None
            #print (bee)
            info = antInfo(direction, bee.velocity, bee.state, 1, False,id)
            self.agentInfoList[bee.id] = info
