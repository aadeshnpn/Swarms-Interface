...
class Bee(HubAgent):
    ...
    def danceTransition(self):  #this runs after the agent has changed state
        dance = int(self.q_value*self.parameters["DanceTime"]-(200*self.assessments))
        if dance < 15:
            self.assessments = 1
...
class Exploring(State):
    def __init__(self, agent=None):
        ...
        self.beacon2Explore = False

    def sense(self, agent, environment):
        agent.checkAgentLeave()
        if not self.beacon2Explore:
            new_q = environment.get_q(agent)["q"]
            agent.q_value = new_q
        elif self.beacon2Explore:
            new_q = 0
            for attractor in environment.flowController.attractors:
                if distance(agent.location, attractor.point) < attractor.radius:
                    new_q = environment.get_q(agent)["q"]
            agent.q_value = new_q
...
class Assessing(State):
    ...
    def update(self, agent):
        ...
        if (distance(agent.location, agent.potential_site) < 1
                    and agent.goingToSite is True):
            agent.goingToSite = False
            return input.siteAssess
        if self.siteFound:
            agent.goingToSite = True
    ...
    def sense(self, agent, environment):
        ...
        if agent.inHub:
            ...
            if isinstance(bee.state, Dancing().__class__):
                if np.random.random()<(bee.q_value*.02):#maybe get rid of the second part
...
class SiteAssess(State):
    ...
    def sense(self, agent, environment):
        ...
        if siteInfo["q"] >= 0 and siteInfo["radius"] > 0: #CHECK THIS, IT MAY BE A PROBLEM...
            ...
            q        = siteInfo["q"]
            if agent.misInformed:
                q = 1-q  # q should be in interval [0,1], so 1-q gives "opposite" quality
            STD_SITE_SIZE = 10
            ...
            adjustedQ = q + agent.priorities["distance"] * distance + agent.priorities["size"] * size
...
class Commit(State):
    ...
    def sense(self, agent, environment):  # probably not needed for now, but can be considered a place holder
        pass
		# environment.hubController.checkOut(agent)
